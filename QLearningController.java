...
public class QLearningController extends Controller {
	...
	void burstTheBigOneAndTurnLeft() {
		leftEngine.setBursting(false);
		rightEngine.setBursting(true);
		middleEngine.setBursting(true);
	} 
	
	void burstTheBigOneAndTurnRight() {
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				//Qtable = Qtable +alpha(Ntable)*(previous_reward+getMaxActionQValue(Qtable.get(prev_stateaction))-Qtable)
				
				double value = Qtable.get(prev_stateaction) + 
